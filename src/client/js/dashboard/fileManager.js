...
export default class FileManagerHandler {
  ...
  onclick (e) {
    ...
    if (isFolder) {
      ...
      const nestedList = parentLi?.querySelector('.nested')
    ...
    const createList = (nodes) => {
      ...
      for (const key in nodes) {
        ...
        if (node.type === 'D') {
          ...
          const folderName = this.createObject('span', '', folderLink, [])
  ...
  insertIntoCurrentFolder (path, isFolder) {
    ...
    const targetPath = this.currentFilePath || ''
  ...
  createObject (tag, id, parent, classNames) {
